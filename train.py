...
parser.add_argument('--seed', type=int, default=20, metavar='S', help='random seed (default: 1)')
...
for epoch in range(start_epoch, args.epochs + 1):
    ...
    # print('train loss is {} and train accuracy is {}'.format(train_res['loss']),train_res['accuracy'])
    if args.curve is None or not has_bn:
        test_res = utils.test(loaders['test'], model, criterion, regularizer)

    if epoch>98 and epoch % args.save_freq == 0:
        utils.save_checkpoint(
            args.dir,
            epoch, name='vgg_res',
            model_state=model.state_dict(),
...
np.savez(
    os.path.join(args.dir, 'train_record_epoch_lr_trainloss_trainaccur_testnll_testaccur' + args.model + '.npz'),
    train_and_value=record)
utils.save_checkpoint(
        args.dir,
        args.epochs, name='wide_resnet_lastepoch',
        model_state=model.state_dict(),
        optimizer_state=optimizer.state_dict()
    )
